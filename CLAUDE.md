...
`TopicExtractor`/`HubBuilder` cap every one-line description (topic and group) to `maxTopicDescriptionWords` (8, in `topic_extractor.go`) so a hub with 20-30 topics still fits SKILL.md's ~500-word budget (per `superpowers:writing-skills`). This is purely a SKILL.md concern — `reference.md` always carries the untruncated text, so nothing is lost, just deferred to the on-demand file. If a future doc set pushes a hub back over budget, tighten this constant (or `firstSentences`' sentence count for the root `Overview`) before reaching for deeper structural changes.
